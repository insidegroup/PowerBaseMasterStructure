...
<div id="contentarea">
	<div id="banner"><div id="banner_text">Price Tracking Hotel Setup</div></div>
		...
		<%using (Html.BeginRouteForm("Default", new { controller = "PriceTrackingSetupGroupItemHotel", action = "Edit",id = Model.PriceTrackingSetupGroupItemHotelId }, FormMethod.Post, new { id = "form0", autocomplete="off" })) {%>
		<%= Html.AntiForgeryToken() %>
			...
			<table cellpadding="0" cellspacing="0" width="100%"> 
				...
				<tr>
					...
					<td><%= Html.TextBoxFor(model => model.ThresholdAmount, new { maxlength = "13", size = "13"})%></td>
				    ...
				    <tr>
					    ...
					    <td colspan="2" style="padding: 0;">
						    <table cellpadding="0" cellspacing="0" border="0" width="100%">
							    <tbody>
								    <% int counter = 0;
								    if (Model.PriceTrackingSetupGroupItemHotelTrackingAlertEmailAddresses != null && Model.PriceTrackingSetupGroupItemHotelTrackingAlertEmailAddresses.Count > 0){ %>
									    <% foreach (CWTDesktopDatabase.Models.PriceTrackingSetupGroupItemHotelTrackingAlertEmailAddress item in Model.PriceTrackingSetupGroupItemHotelTrackingAlertEmailAddresses) { %>
										    <tr class="HotelTrackingAlertsEmailAddresses_Line_Item">
											    <td width="30%">
												    <%= Html.TextBox("PriceTrackingSetupGroupItemHotelTrackingAlertEmailAddress[" + counter + "].EmailAddress", item.EmailAddress, new { maxlength="100", size = "22", @Class = "HotelTrackingAlertsEmailAddress" })%>
											    </td>
											    <td width="70%">
						    ...
						    <table cellpadding="0" cellspacing="0" border="0" width="100%">
							    <tbody>
									    ...
									    <% foreach (CWTDesktopDatabase.Models.PriceTrackingSetupGroupItemHotelTrackingAlertEmailAddress item in Model.PriceTrackingSetupGroupItemHotelTrackingAlertEmailAddresses) { %>
										    <tr class="HotelTrackingAlertsEmailAddresses_Line_Item">
											    <td width="100%">
                                                    ...
                                                    <%= Html.Hidden("PriceTrackingSetupGroupItemHotelTrackingAlertEmailAddress[" + counter + "].EmailAddress", item.EmailAddress)%>
											    </td>
										    </tr>
									    <% counter++;
									    } %>
								    <% } else { %>
									    <tr class="HotelTrackingAlertsEmailAddresses_Line_Item">
										    <td width="100%">
											    <%= Html.Hidden("PriceTrackingSetupGroupItemHotelTrackingAlertEmailAddress[0].EmailAddress", "")%>
										    </td>
									    </tr>
								    <% } %>
							    </tbody>
			...
			<%= Html.HiddenFor(model => model.PriceTrackingSetupGroupItemHotelId) %>
		<% } %>
		</div>
	</div>
