    ...
    <div id="banner"><div id="banner_text">Client SubUnit - Credit Cards</div></div>
        ...
        <% using (Html.BeginForm()) {%>
            ...
            <%= Html.ValidationSummary(true) %>
				...
				<tr>
					<td>
						<label for="ProductId">Product</label></td>
            ...
            <%= Html.HiddenFor(model => model.ClientSubUnitGuid)%>
             <%= Html.Hidden("HierarchyType", "ClientSubUnit")%>
            <%= Html.Hidden("HierarchyItem", Model.ClientSubUnitName)%>
            <%= Html.HiddenFor(model => model.ClientTopUnitGuid)%>

    <% } %>
        </div>
        </div>
    <script src="<%=Url.Content("~/Scripts/ERD/CreditCard2Edit.js")%>" type="text/javascript"></script>
</asp:Content>
