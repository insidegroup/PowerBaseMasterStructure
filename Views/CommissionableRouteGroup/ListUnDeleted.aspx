<%@ Page Title="" Language="C#" MasterPageFile="~/Views/Shared/Site.Master" Inherits="System.Web.Mvc.ViewPage<CWTDesktopDatabase.Helpers.CWTPaginatedList<CWTDesktopDatabase.Models.spDesktopDataAdmin_SelectCommissionableRouteGroups_v1Result>>" %>
...
<div id="contentarea">
    <div id="banner"><div id="banner_text">Commissionable Route Groups</div></div>
        <div id="content">
            <table cellpadding="0" cellspacing="0" border="0" style="width: 100%; table-layout: fixed;">
                <tr> 
			        <th width="40%" class="row_header_left"><%= Html.RouteLink("Group Name", "ListMain", new { page =1, sortField = "CommissionableRouteGroupName", sortOrder = ViewData["NewSortOrder"].ToString(), filter = Request.QueryString["filter"] })%></th> 
                ...
                <tr>
                    ...
                    <td>
                        <%if((bool)item.HasWriteAccess.Value == true){%>
                        <%=  Html.ActionLink("Delete", "Delete", new { id = item.CommissionableRouteGroupId }, new { title = "Delete" })%>
                        <%} %>
                    </td>
                </tr>        
                <% 
		        ...
		        <tr>
                    <td colspan="7" class="row_footer_blank_left row_footer_blank_left_no_padding">
						<a href="javascript:window.print();" class="red btn-small" title="Print">Print</a> 
						<%= Html.ActionLink("Orphaned Commissionable Route Groups", "ListOrphaned", null, new { @class = "red btn-small", title="Orphaned Commissionable Route Groups" })%>&nbsp;
						<%= Html.ActionLink("Deleted Commissionable Route Groups", "ListDeleted", null, new { @class = "red btn-small", title="Deleted Commissionable Route Groups"})%>&nbsp;
						<%if (ViewData["Access"] == "WriteAccess"){ %>
							<%= Html.ActionLink("Create Commissionable Route Group", "Create", null, new { @class = "red btn-small" ,title="Create Commissionable Route Group" })%>
						<%} %>
                    </td>
		        </tr> 
            </table>
...
<script type="text/javascript">
    ...
    $('#btnSearch').click(function() {
      ...
      $("#frmSearch").attr("action", "/CommissionableRouteGroup.mvc/ListUnDeleted");
      $("#frmSearch").submit();
