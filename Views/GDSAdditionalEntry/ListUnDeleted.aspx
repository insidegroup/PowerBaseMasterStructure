    ...
    <div id="banner"><div id="banner_text">GDS Additional Entries</div></div>
        <div id="content">
            <table cellpadding="0" cellspacing="0" border="0" style="width: 100%; table-layout: fixed;">
                <tr> 
			        <th width="23%" class="row_header_left"><%= Html.RouteLink("Value", "ListMain", new { page = 1, sortField = "GDSAdditionalEntryValue", sortOrder = ViewData["NewSortOrder"].ToString(), filter = Request.QueryString["filter"] })%></th> 
			        <th width="30%"></th> 
                ...
                %>
                <tr>
                    <td colspan="2"><%= Html.Encode(CWTStringHelpers.TrimString(item.GDSAdditionalEntryValue, 60))%></td>
                    ...
                    <td>
                        <%if(item.HasWriteAccess.Value){%>
                        <%=  Html.ActionLink("Delete", "Delete", new { id = item.GDSAdditionalEntryId }, new { title = "Delete" })%>
                        <%} %>
                    </td>
                </tr>        
                <% 
               ...
               <tr>
		        ...
		        <tr> 
			        ...
			        <td class="row_footer_blank_right" colspan="6"><%= Html.ActionLink("Orphaned GDS Additional Entries", "ListOrphaned", null, new { @class = "red", title="Orphaned GDS Additional Entriees" })%>&nbsp;<%= Html.ActionLink("Deleted GDS Additional Entries", "ListDeleted", null, new { @class = "red", title = "Deleted GDS Additional Entries" })%>
			        <%if (ViewData["Access"] == "WriteAccess"){ %>&nbsp;<%= Html.ActionLink("Create GDS Additional Entry", "Create", null, new { @class = "red", title = "Create GDS Additional Entry" })%><%} %></td> 
		        </tr> 
            </table>
        </div>
    </div>
<script type="text/javascript">
    ...
    $('#btnSearch').click(function() {
        ...
        $("#frmSearch").attr("action", "/GDSAdditionalEntry.mvc/ListUnDeleted");
        $("#frmSearch").submit();
