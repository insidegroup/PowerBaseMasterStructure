...
$(document).ready(function () {
	...
	$("#HierarchyType").change(function () {
		...
		$("#HierarchyItem").val("");
		if ($("#CommissionableRouteGroupId").val() == "0") {
			$("#lblAuto").text("");
			$("#CommissionableRouteGroupName").val("");
		}
		if ($("#HierarchyType").val() == "") {
			$("#HierarchyItem").attr("disabled", true);
	...
	$('#form0').submit(function () {
		...
		//	}
		//} else {
		//	if (jQuery.trim($("#CommissionableRouteGroupName").val()) == "") {
		//		$("#CommissionableRouteGroupName_validationMessage").removeClass('field-validation-valid');
		//		$("#CommissionableRouteGroupName_validationMessage").addClass('field-validation-error');
		//		$("#CommissionableRouteGroupName_validationMessage").text("Commissionable Route Group Name Required.");
		//		return false;
		//	} else {
		//		$("#CommissionableRouteGroupName_validationMessage").text("");
		//	}
		//}
		...
		jQuery.ajax({
			type: "POST",
			url: "/GroupNameBuilder.mvc/IsAvailableCommissionableRouteGroupName",
			data: { groupName: $("#CommissionableRouteGroupName").val(), id: $("#CommissionableRouteGroupId").val() },
			success: function (data) {
				validGroupName = data;
			},
			dataType: "json",
		...
		if (!validGroupName) {
			...
			$("#lblCommissionableRouteGroupNameMsg").removeClass('field-validation-valid');
			$("#lblCommissionableRouteGroupNameMsg").addClass('field-validation-error');
			if ($("#CommissionableRouteGroupId").val() == "0") {//Create
				$("#lblCommissionableRouteGroupNameMsg").text("This name has already been used, please reselect " + $("#lblHierarchyItem").text() + " to update the name.");
			} else {
				if ($("#CommissionableRouteGroupName").val() != "") {
					$("#lblCommissionableRouteGroupNameMsg").text("This name has already been used, please choose a different name.");
				}
			}
			return false;
		} else {
			$("#lblCommissionableRouteGroupNameMsg").text("");
		}
		//GroupName End
	...
	$("#HierarchyItem").autocomplete({
		source: function (request, response) {
			...
			if ($("#HierarchyType").val() == "ClientSubUnitTravelerType") {
				$.ajax({
					url: "/CommissionableRouteGroup.mvc/AutoCompleteClientSubUnitTravelerTypes", type: "POST", dataType: "json",
				...
				$.ajax({
					...
					data: { searchText: request.term, hierarchyItem: "TravelerType", domainName: 'Commissionable Route', resultCount: 5000 },
					success: function (data) {
						response($.map(data, function (item) {
							return {
				...
				$.ajax({
					url: "/AutoComplete.mvc/Hierarchies", type: "POST", dataType: "json",
					data: { searchText: request.term, hierarchyItem: $("#HierarchyType").val(), domainName: 'Commissionable Route', resultCount: 5000 },
					success: function (data) {
						response($.map(data, function (item) {
							if (
			...
			if ($("#CommissionableRouteGroupId").val() == "0") {//Create
				...
				if ($("#HierarchyType").val() == "ClientAccount") {
					$.ajax({
						...
						data: { clientAccountNumber: $("#HierarchyCode").val(), sourceSystemCode: $("#SourceSystemCode").val(), group: "Commissionable Route" },
						success: function (data) {
							var maxNameSize = 50 - (htft.length + 16);
							var autoName = replaceSpecialChars(ui.item.value)
							autoName = autoName.substring(0, maxNameSize) + "_" + data + "_" + htft + "_CommissionableRoute";
							$("#lblAuto").text(autoName);
							$("#CommissionableRouteGroupName").val(autoName);
							$("#lblCommissionableRouteGroupNameMsg").text("");
						}
					})
				} else {
					$.ajax({
						...
						data: { hierarchyType: $("#HierarchyType").val(), hierarchyItem: $("#HierarchyCode").val(), group: "Commissionable Route" },
						success: function (data) {
