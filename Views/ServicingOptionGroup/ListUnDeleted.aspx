...
<div id="contentarea">
    <div id="banner"><div id="banner_text">Servicing Option Groups</div></div>
        <div id="content">
           <table cellpadding="0" cellspacing="0" border="0" style="width: 100%; table-layout: fixed;">
                <tr> 
			        <th width="24%" class="row_header_left"><%= Html.RouteLink("Group Name", "ListMain", new { page = 1, sortField = "ServicingOptionGroupName", sortOrder = ViewData["NewSortOrder"].ToString(), filter = Request.QueryString["filter"] })%></th> 
			        <th width="19%">&nbsp;</th>
                ...
                %>
                <tr>
                    <td colspan="2" class="wrap-text"><%= Html.Encode(item.ServicingOptionGroupName) %></td>
                    ...
                    <td>
                        <%if(item.HasWriteAccess.Value){%>
                        <%=  Html.RouteLink("Delete", "Default", new { id = item.ServicingOptionGroupId, action = "Delete" }, new { title = "Delete" })%>
                        <%} %>
                    </td>
                </tr>        
                <% 
		        ...
		        <tr> 
			        ...
			        <td class="row_footer_blank_right" colspan="8"><%= Html.ActionLink("Orphaned Servicing Option Groups", "ListOrphaned", null, new { @class = "red", title="Orphaned Servicing Option Groups" })%>&nbsp;<%= Html.ActionLink("Deleted Servicing Option Groups", "ListDeleted", null, new { @class = "red" })%>
			        <%if (ViewData["Access"] == "WriteAccess"){ %>&nbsp;<%= Html.ActionLink("Create Servicing Option Group", "Create", null, new { @class = "red" })%><%} %></td> 
		        </tr> 
            </table>
...
<script type="text/javascript">
    ...
    $('#btnSearch').click(function() {
        ...
        $("#frmSearch").attr("action", "/ServicingOptionGroup.mvc/ListUnDeleted");
        $("#frmSearch").submit();
