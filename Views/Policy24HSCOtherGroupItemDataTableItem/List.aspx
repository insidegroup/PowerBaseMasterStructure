<%@ Page Title="" Language="C#" MasterPageFile="~/Views/Shared/SiteFullWidth.Master" Inherits="System.Web.Mvc.ViewPage<CWTDesktopDatabase.ViewModels.Policy24HSCOtherGroupItemDataTableItemsVM>" %>
    ...
    <div id="banner"><div id="banner_text">Table Data Items</div></div>
        <div id="content">
			<table width="100%" cellpadding="0" cellspacing="0" border="0">
               <tr> 
			        <th class="row_header row_header_white" colspan="<%= Html.Encode(Model.Policy24HSCOtherGroupItemDataTableItems.Columns.Count + 1) %>">
						Table Name - <%= Html.Encode(Model.PolicyOtherGroupHeader.TableName) %>
			        </th> 
		        </tr>
				<tr>
					<% foreach (System.Data.DataColumn col in Model.Policy24HSCOtherGroupItemDataTableItems.Columns) { 
						if(col.ColumnName != "Policy24HSCOtherGroupItemDataTableRowId") { %>
				...
				<% foreach (System.Data.DataRow row in Model.Policy24HSCOtherGroupItemDataTableItems.Rows) { %>
					<tr>
						<% foreach (System.Data.DataColumn col in Model.Policy24HSCOtherGroupItemDataTableItems.Columns) { 
								if(col.ColumnName != "Policy24HSCOtherGroupItemDataTableRowId") { %>           
						...
						<td width="8%">
							<%if(Model.HasWriteAccess){%>
								<%=Html.RouteLink("Edit", "Default", new { action = "Edit", id = Html.Encode(row["Policy24HSCOtherGroupItemDataTableRowId"]), policyGroupId = Model.PolicyGroup.PolicyGroupId, PolicyOtherGroupHeaderId = Model.PolicyOtherGroupHeader.PolicyOtherGroupHeaderId }, new { title = "Edit" })%>
							<%} %>
						</td>
						<td width="10%">
							<%if(Model.HasWriteAccess){%>
								<%=Html.RouteLink("Delete", "Default", new { action = "Delete", id = Html.Encode(row["Policy24HSCOtherGroupItemDataTableRowId"]), policyGroupId = Model.PolicyGroup.PolicyGroupId, PolicyOtherGroupHeaderId = Model.PolicyOtherGroupHeader.PolicyOtherGroupHeaderId }, new { title = "Delete" })%>
							<%} %>
						</td>
					</tr>
				<% } %>
                <tr>
                    <td colspan="<%= Html.Encode(Model.Policy24HSCOtherGroupItemDataTableItems.Columns.Count + 1) %>" class="row_footer">
		        ...
		        <tr> 
					<td class="row_footer_blank_left" colspan="<%= Model.Policy24HSCOtherGroupItemDataTableItems.Columns.Count - 1 %>">
...
<script type="text/javascript">
	...
	$('#btnSearch').click(function () {
		...
		$("#frmSearch").attr("action", "/Policy24HSCOtherGroupItemDataTableItem.mvc/List");
		$("#frmSearch").submit();
	});
</script>
</asp:Content>
