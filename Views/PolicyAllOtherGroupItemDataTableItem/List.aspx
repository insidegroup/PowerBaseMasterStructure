<%@ Page Title="" Language="C#" MasterPageFile="~/Views/Shared/SiteFullWidth.Master" Inherits="System.Web.Mvc.ViewPage<CWTDesktopDatabase.ViewModels.PolicyAllOtherGroupItemDataTableItemsVM>" %>
    ...
    <div id="banner"><div id="banner_text">Table Data Items</div></div>
        <div id="content">
			<table width="100%" cellpadding="0" cellspacing="0" border="0">
               <tr> 
			        <th class="row_header row_header_white" colspan="<%= Html.Encode(Model.PolicyAllOtherGroupItemDataTableItems.Columns.Count + 1) %>">
						Table Name - <%= Html.Encode(Model.PolicyOtherGroupHeader.TableName) %>
			        </th> 
		        </tr>
				<tr>
					<% foreach (System.Data.DataColumn col in Model.PolicyAllOtherGroupItemDataTableItems.Columns) { 
						if(col.ColumnName != "PolicyAllOtherGroupItemDataTableRowId") { %>
				...
				<% foreach (System.Data.DataRow row in Model.PolicyAllOtherGroupItemDataTableItems.Rows) { %>
					<tr>
						<% foreach (System.Data.DataColumn col in Model.PolicyAllOtherGroupItemDataTableItems.Columns) { 
								if(col.ColumnName != "PolicyAllOtherGroupItemDataTableRowId") { %>           
						...
						<td width="8%">
							<%if(Model.HasWriteAccess){%>
								<%=Html.RouteLink("Edit", "Default", new { action = "Edit", id = Html.Encode(row["PolicyAllOtherGroupItemDataTableRowId"]), policyGroupId = Model.PolicyGroup.PolicyGroupId, PolicyOtherGroupHeaderId = Model.PolicyOtherGroupHeader.PolicyOtherGroupHeaderId }, new { title = "Edit" })%>
							<%} %>
						</td>
						<td width="10%">
							<%if(Model.HasWriteAccess){%>
								<%=Html.RouteLink("Delete", "Default", new { action = "Delete", id = Html.Encode(row["PolicyAllOtherGroupItemDataTableRowId"]), policyGroupId = Model.PolicyGroup.PolicyGroupId, PolicyOtherGroupHeaderId = Model.PolicyOtherGroupHeader.PolicyOtherGroupHeaderId }, new { title = "Delete" })%>
							<%} %>
						</td>
					</tr>
				<% } %>
                <tr>
                    <td colspan="<%= Html.Encode(Model.PolicyAllOtherGroupItemDataTableItems.Columns.Count + 1) %>" class="row_footer">
		        ...
		        <tr> 
					<td class="row_footer_blank_left" colspan="<%= Model.PolicyAllOtherGroupItemDataTableItems.Columns.Count - 1 %>">
...
<script type="text/javascript">
	...
	$('#btnSearch').click(function () {
		...
		$("#frmSearch").attr("action", "/PolicyAllOtherGroupItemDataTableItem.mvc/List");
		$("#frmSearch").submit();
	});
</script>
</asp:Content>
...
<%=Html.RouteLink(Model.PolicyOtherGroupHeader.Label, "Default", new { controller = "PolicyAllOtherGroupItem", action = "List", id = Model.PolicyGroup.PolicyGroupId }, new { title = Model.PolicyOtherGroupHeader.Label })%> &gt;
<%=Html.RouteLink(Model.PolicyOtherGroupHeader.TableName, "Default", new { controller = "PolicyAllOtherGroupItem", action = "List", id = Model.PolicyGroup.PolicyGroupId }, new { title = Model.PolicyOtherGroupHeader.TableName })%> &gt;
Table Data
</asp:Content>