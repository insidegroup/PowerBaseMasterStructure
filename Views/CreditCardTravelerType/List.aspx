<%@ Page Title="" Language="C#" MasterPageFile="~/Views/Shared/SiteFullWidth.Master" Inherits="System.Web.Mvc.ViewPage<CWTDesktopDatabase.ViewModels.TravelerTypeCreditCardsVM>" %>
...
<div id="contentarea">
    <div id="banner"><div id="banner_text">TravelerType - Credit Cards</div></div>
    <div id="content">
		...
		<table width="100%" cellpadding="0" cellspacing="0">
            <tr> 
		        <th width="16%" class="row_header_left"><%= Html.RouteLink("Card Type", "ListMain", new { controller = "CreditCardTravelerType", action = "List", page = 1, filter = Request.QueryString["filter"], tt = Model.TravelerType.TravelerTypeGuid, csu = Model.ClientSubUnit.ClientSubUnitGuid, sortField = "CreditCardTypeDescription", sortOrder = ViewData["NewSortOrder"].ToString() }, new { title = "Card Type" })%></th>
				<th width="10%"><%= Html.RouteLink("Product", "ListMain", new { controller = "CreditCardTravelerType", action = "List", page = 1, filter = Request.QueryString["filter"], tt = Model.TravelerType.TravelerTypeGuid, csu = Model.ClientSubUnit.ClientSubUnitGuid, sortField = "ProductName", sortOrder = ViewData["NewSortOrder"].ToString() }, new { title = "Product" })%></th>
				<th width="17%"><%= Html.RouteLink("Supplier", "ListMain", new { controller = "CreditCardTravelerType", action = "List", page = 1, filter = Request.QueryString["filter"], tt = Model.TravelerType.TravelerTypeGuid, csu = Model.ClientSubUnit.ClientSubUnitGuid, sortField = "SupplierName", sortOrder = ViewData["NewSortOrder"].ToString() }, new { title = "Supplier" })%></th>
				<th width="10%"><%= Html.RouteLink("Card Holder", "ListMain", new { controller = "CreditCardTravelerType", action = "List", page = 1, filter = Request.QueryString["filter"], tt = Model.TravelerType.TravelerTypeGuid, csu = Model.ClientSubUnit.ClientSubUnitGuid, sortField = "CreditCardHolderName", sortOrder = ViewData["NewSortOrder"].ToString() }, new { title = "Card Holder" })%></th> 
		        <th width="10%"><%= Html.RouteLink("Card Number", "ListMain", new { controller = "CreditCardTravelerType", action = "List", page = 1, filter = Request.QueryString["filter"], tt = Model.TravelerType.TravelerTypeGuid, csu = Model.ClientSubUnit.ClientSubUnitGuid, sortField = "MaskedCreditCardNumber", sortOrder = ViewData["NewSortOrder"].ToString() }, new { title = "Card Number" })%></th> 
		        <th width="5%">CVV</th>
            ...
            <tr>
                ...
                <td><%= Html.RouteLink("View", "Main", new { controller = "CreditCardTravelerType", action = "View", id = item.CreditCardId, csu = Model.ClientSubUnit.ClientSubUnitGuid }, new { title = "View" })%> </td>
                <%if(Model.CreditCardBehavior.CanChangeCreditCardsFlag){ %>
                <td>
                    <%if (ViewData["Access"] == "WriteAccess")
                      {%>
                        <%= Html.RouteLink("Edit", "Main", new { controller = "CreditCardTravelerType", action = "Edit", id = item.CreditCardId, csu = Model.ClientSubUnit.ClientSubUnitGuid }, new { title = "Edit" })%>
                    <%} %>
                </td>
                 <td>
                    <%if (ViewData["Access"] == "WriteAccess")
                      {%>
                        <%= Html.RouteLink("Delete", "Main", new { controller = "CreditCardTravelerType", action = "Delete", id = item.CreditCardId, csu = Model.ClientSubUnit.ClientSubUnitGuid }, new { title = "Remove" })%>
                    <%} %>
                </td>
               <%} %>
            ...
            <tr>
                    ...
                    <div class="paging_container">
                        ...
                        <%=  Html.RouteLink("<<Previous Page", "ListMain", new { controller = "CreditCardTravelerType", action = "List", csu = Model.ClientSubUnit.ClientSubUnitGuid, tt = Model.TravelerType.TravelerTypeGuid, page = (Model.CreditCards.PageIndex - 1), sortField = ViewData["CurrentSortField"].ToString(), sortOrder = ViewData["NewSortOrder"].ToString() }, new { title = "Previous Page" })%>
                        <%}%>
                        </div>
                        <div class="paging_right">
                        <% if (Model.CreditCards.HasNextPage){  %>
                        <%=  Html.RouteLink("Next Page>>", "ListMain", new { controller = "CreditCardTravelerType", action = "List", csu = Model.ClientSubUnit.ClientSubUnitGuid, tt = Model.TravelerType.TravelerTypeGuid, page = (Model.CreditCards.PageIndex + 1), sortField = ViewData["CurrentSortField"].ToString(), sortOrder = ViewData["NewSortOrder"].ToString() }, new { title = "Next Page" })%>
                        <%}%> 
                        </div>
	        ...
	        <tr> 
		        ...
		        <%if (ViewData["Access"] == "WriteAccess"){ %>
		        <%= Html.RouteLink("Add Credit Card", "Main", new { controller = "CreditCardTravelerType", action = "Create", csu = Model.ClientSubUnit.ClientSubUnitGuid, tt = Model.TravelerType.TravelerTypeGuid }, new { @class = "red", title = "Add Credit Card" })%>
		        <% } %> 
		        </td> 
	        </tr> 
        </table>
...
<%=Html.RouteLink(Model.TravelerType.TravelerTypeName, "Main", new { controller = "TravelerType", action = "ViewItem", id = Model.TravelerType.TravelerTypeGuid, csu = Model.ClientSubUnit.ClientSubUnitGuid }, new { title = Model.TravelerType.TravelerTypeName })%> &gt;
TT Credit Cards</asp:Content>