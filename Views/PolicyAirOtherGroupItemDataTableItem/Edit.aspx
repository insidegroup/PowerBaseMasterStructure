<%@ Page Title="" Language="C#" MasterPageFile="~/Views/Shared/SiteFullWidth.Master" Inherits="System.Web.Mvc.ViewPage<CWTDesktopDatabase.ViewModels.PolicyAirOtherGroupItemDataTableItemVM>" %>
...
<div id="contentarea">
        ...
        <div id="content">
			...
			<% using (Html.BeginForm()) {%>
				...
				<table cellpadding="0" border="0" width="100%" cellspacing="0"> 
					...
					<% int counter = 1;
					foreach (var item in Model.PolicyAirOtherGroupItemDataTableItems.OrderBy(x => x.PolicyOtherGroupHeaderColumnName.DisplayOrder)) { %>
						<tr>
							<td><label for="PolicyAirOtherGroupItemLanguage_Label"><% =Html.Encode(item.PolicyOtherGroupHeaderColumnName.ColumnName)%></label></td>
							<td><%= Html.TextArea("PolicyOtherGroupHeaderColumnNameId_" + item.PolicyOtherGroupHeaderColumnNameId.ToString(), 
									...
									new { 
										...
										@class="PolicyAirOtherGroupItemDataTableItem"
									})
								%>
							</td>
							<td>
								...
								<% if(counter == 1) { %>
									<span class="error" id="PolicyAirOtherGroupItemDataTableItemErrorMessage"></span>
								<% } %>
							</td>
						</tr> 
					<% counter++; } %>
					...
					<tr>
						...
						<td class="row_footer_blank_right">
							...
							<%= Html.HiddenFor(model => model.PolicyAirOtherGroupItemDataTableRow.PolicyAirOtherGroupItemDataTableRowId)%>
							<%= Html.HiddenFor(model => model.PolicyAirOtherGroupItemDataTableRow.VersionNumber)%>
	...
	<script src="<%=Url.Content("~/Scripts/ERD/PolicyAirOtherGroupItemDataTableItem.js")%>" type="text/javascript"></script>
	<script type="text/javascript">
		$(document).ready(function () {
			<% foreach(var item in Model.PolicyAirOtherGroupItemDataTableItems) { %>
...
<%=Html.RouteLink("Table Data", "Default", new { controller = "PolicyAirOtherGroupItemDataTableItem", action = "List", id = Model.PolicyOtherGroupHeader.PolicyOtherGroupHeaderId, policyGroupId = Model.PolicyGroup.PolicyGroupId }, new { title = "Table Data" })%> &gt;
Edit
</asp:Content>