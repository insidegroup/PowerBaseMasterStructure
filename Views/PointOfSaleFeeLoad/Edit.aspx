     ...
     <div id="banner"><div id="banner_text">Point of Sale Fee Definitions</div></div>
        ...
        <%= Html.AntiForgeryToken() %>
            ...
            <table cellpadding="0" border="0" width="100%" cellspacing="0" class="main-table"> 
		        <tr> 
			        <th class="row_header" colspan="3">Edit Point of Sale Fee Definition</th> 
		        </tr> 
		        <tr>
                    <td><label for="PointOfSaleFeeLoad_ClientTopUnitGuid">Client TopUnit Name</label></td>
                    <td><%= Html.Raw(Model.PointOfSaleFeeLoad.ClientTopUnitName)%> </td>
                    <td>
                        <label id="lblPointOfSaleFeeLoad_ClientTopUnitGuid_Msg"/>
                        <label id="PointOfSaleFeeLoad_Error"/>
                    </td>
			    </tr>  
                <tr>
                    <td><label for="PointOfSaleFeeLoad_ClientSubUnitGuid">Client SubUnit Name</label></td>
                    <td><%= Html.Raw(Model.PointOfSaleFeeLoad.ClientSubUnitName)%></td>
                    <td><label id="lblPointOfSaleFeeLoad_ClientSubUnitGuid_Msg"/></td>
			    </tr>  
                <tr>
                    <td><label for="PointOfSaleFeeLoad_TravelerTypeGuid">Traveler Type</label></td>
                    <td><%= Html.Raw(Model.PointOfSaleFeeLoad.TravelerTypeName)%></td>
                    <td><label id="lblPointOfSaleFeeLoad_TravelerTypeGuid_Msg"/></td>
			    </tr>
                <tr>
                ...
                <tr>
                    ...
                    <td>
                        <%= Html.TextBoxFor(model => model.PointOfSaleFeeLoad.FeeLoadAmount, new { autocomplete="off", @Class = "FeeLoadAmountEdit" })%><span class="error"> *</span>
			    ...
			    <tr>
				    ...
				    <td class="row_footer_blank_right" colspan="2"><input type="submit" value="Edit POS Fee" class="red" title="Edit POS Fee"/></td>
			    </tr>
		    </table>

            <%= Html.HiddenFor(model => model.PointOfSaleFeeLoad.ClientTopUnitGuid)%>
            <%= Html.HiddenFor(model => model.PointOfSaleFeeLoad.ClientSubUnitGuid)%>
            <%= Html.HiddenFor(model => model.PointOfSaleFeeLoad.TravelerTypeGuid)%>
