<%@ Control Language="C#" Inherits="System.Web.Mvc.ViewUserControl<IEnumerable<CWTDesktopDatabase.Models.spDesktopDataAdmin_SelectClientTopUnitSubUnits_v1Result>>" %>
<style>
tr.over td
 ...
 <%if (Model.Count() > 0)
   { %>
    ...
    <% foreach (var item in Model) { %>
        ...
        <tr>
			<td <%= Html.Raw((item.ExpiryDate != null && item.ExpiryDate.Value <= DateTime.Now) || (item.ClientTopUnitExpiryDate != null && item.ClientTopUnitExpiryDate.Value <= DateTime.Now) ? " class=\"expired\"" : String.Empty) %>>
