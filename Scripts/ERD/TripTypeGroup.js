...
$(document).ready(function() {
    ...
    $("#HierarchyType").change(function() {
        ...
        $("#HierarchyItem").val("");
        if ($("#TripTypeGroupId").val() == "0") {
            $("#lblAuto").text("");
            $("#TripTypeGroupName").val("");
        }
        if ($("#HierarchyType").val() == "") {
            $("#HierarchyItem").attr("disabled", true);
    ...
    */
    $(function() {
        $("#HierarchyItem").autocomplete({
            source: function(request, response) {
                ...
                if ($("#HierarchyType").val() == "ClientSubUnitTravelerType") {
                	$.ajax({
                		url: "/TripTypeGroup.mvc/AutoCompleteClientSubUnitTravelerTypes", type: "POST", dataType: "json",
                	...
                	$.ajax({
                		...
                		data: { searchText: request.term, hierarchyItem: "TravelerType", domainName: 'Trip Types', resultCount: 5000 },
                		success: function (data) {
                			response($.map(data, function (item) {
                				return {
                	...
                	$.ajax({
                		url: "/AutoComplete.mvc/Hierarchies", type: "POST", dataType: "json",
                		data: { searchText: request.term, hierarchyItem: $("#HierarchyType").val(), domainName: 'Trip Types', resultCount: 5000 },
                		success: function (data) {
                			response($.map(data, function (item) {
                				if (
            ...
            select: function(event, ui) {
                ...
                if ($("#TripTypeGroupId").val() == "0") {//Create
                    ...
                    if ($("#HierarchyType").val() == "ClientAccount") {
                        $.ajax({
                            ...
                            data: { clientAccountNumber: $("#HierarchyCode").val(), sourceSystemCode: $("#SourceSystemCode").val(), group: "Trip Types" },
                            success: function(data) {
                                var maxNameSize = 50 - ($("#HierarchyType").val().length + 14);
                                var autoName = replaceSpecialChars(ui.item.value)
                                autoName = autoName.substring(0, maxNameSize) + "_" + data + "_" + $("#HierarchyType").val() + "_TripType";
                                $("#lblAuto").text(autoName);
                                $("#TripTypeGroupName").val(autoName);
                                $("#lblTripTypeGroupNameMsg").text("");
                            }
                        })
                    } else {
                        $.ajax({
                            url: "/GroupNameBuilder.mvc/BuildGroupName", type: "POST", dataType: "json",
                            data: { hierarchyType: $("#HierarchyType").val(), hierarchyItem: $("#HierarchyCode").val(), group: "Trip Types" },
                            success: function(data) {
                                var maxNameSize = 50 - ($("#HierarchyType").val().length + 14);
                                var autoName = replaceSpecialChars(ui.item.value)
                                autoName = autoName.substring(0, maxNameSize) + "_" + data + "_" + $("#HierarchyType").val() + "_TripType";
    ...
    $('#form0').submit(function () {
        ...
        if ($("#TripTypeGroupId").val() == "0") {
            ...
            if (jQuery.trim($("#TripTypeGroupName").val()) == "") {
                $("#TripTypeGroupName_validationMessage").removeClass('field-validation-valid');
                $("#TripTypeGroupName_validationMessage").addClass('field-validation-error');
                $("#TripTypeGroupName_validationMessage").text("Trip Type Name Required.");
                return false;
            } else {
                $("#TripTypeGroupName_validationMessage").text("");
            }
        }
        ...
        jQuery.ajax({
            type: "POST",
            url: "/GroupNameBuilder.mvc/IsAvailableTripTypeGroupName",
            data: { groupName: $("#TripTypeGroupName").val(), id: $("#TripTypeGroupId").val() },
            success: function(data) {
        ...
        if (!validGroupName) {
            ...
            $("#lblTripTypeGroupNameMsg").removeClass('field-validation-valid');
            $("#lblTripTypeGroupNameMsg").addClass('field-validation-error');
            if ($("#TripTypeGroupId").val() == "0") {//Create
                $("#lblTripTypeGroupNameMsg").text("This name has already been used, please reselect " + $("#lblHierarchyItem").text() + " to update the name.");
            } else {
                if ($("#TripTypeGroupName").val() != "") {
                    $("#lblTripTypeGroupNameMsg").text("This name has already been used, please choose a different name.");
                }
            }
            return false;
        } else {
