            ...
            <tr>
				...
				<td>
                    <%if(ViewData["Access"] == "WriteAccess" && item.BackOfficeDataSourceId == 2){%>
						<%= Html.RouteLink("Edit", "Main", new { action = "Edit", id = item.ClientDefinedReferenceItemValueId, csu = ViewData["ClientSubUnitGuid"].ToString() }, new { title = "Edit" })%>                     
                    <%}%>
                </td>
                <td>
                    <%if(ViewData["Access"] == "WriteAccess" && item.BackOfficeDataSourceId == 2){%>
                    <%= Html.RouteLink("Delete", "Main", new { action = "Delete", id = item.ClientDefinedReferenceItemValueId, csu = ViewData["ClientSubUnitGuid"].ToString()}, new { title = "Delete" })%>
                    <%}%>
                </td>
            </tr>        
            <% 
            ...
            <tr>
                <td colspan="4" class="row_footer">
                    <div class="paging_container">
                        ...
                        <%=  Html.RouteLink("<<Previous Page", "ListMain", new { action = "List", id = ViewData["ClientDefinedReferenceItemId"].ToString(), page = (Model.PageIndex - 1), sortField = ViewData["CurrentSortField"], sortOrder = ViewData["CurrentSortOrder"].ToString(), filter = Request.QueryString["filter"] }, new { title = "Previous Page" })%>
                        <%}%>
                        </div>
                        <div class="paging_right">
                        <% if (Model.HasNextPage){  %>
                        <%=  Html.RouteLink("Next Page>>", "ListMain", new { action = "List", id = ViewData["ClientDefinedReferenceItemId"].ToString(), page = (Model.PageIndex + 1), sortField = ViewData["CurrentSortField"], sortOrder = ViewData["CurrentSortOrder"].ToString(), filter = Request.QueryString["filter"] }, new { title = "Next Page" })%>
                        <%}%> 
                        </div>
	        ...
	        <tr> 
		        ...
		        <td class="row_footer_blank_right" colspan="3">
					<%if(ViewData["Access"] == "WriteAccess" && ViewData["CreateAccess"] == "CreateAccess"){%>
						<%= Html.RouteLink("Create Drop List Item", "Main", new { action="Create", id = ViewData["ClientDefinedReferenceItemId"].ToString(), csu = ViewData["ClientSubUnitGuid"].ToString() }, new { @class = "red", title = "Create Drop List Item" })%>
					<% } %>
		        </td> 
	        </tr> 
        </table>
...
<script type="text/javascript">
    $(document).ready(function() {
        ...
        $('#search').show();
        $('#ft').attr('name', 'id');
        $("#frmSearch input[name='id']")
			.val('<%=Html.Encode(ViewData["ClientDefinedReferenceItemId"].ToString())%>')
    		.after('<input type="hidden" name="csu" value="<%=Html.Encode(ViewData["ClientSubUnitGuid"].ToString())%>"/>');

        $('#btnSearch').click(function () {
        	$("#frmSearch").attr("action", "/ClientSubUnitClientDefinedReferenceItemValue.mvc/List");
        	$("#frmSearch").submit();
        });
    })
 </script>
