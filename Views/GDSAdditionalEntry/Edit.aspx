...
<div id="contentarea">
    ...
    <% Html.EnableClientValidation(); %>
        ...
        <%using (Html.BeginRouteForm("Default", new { controller = "GDSAdditionalEntry", action = "Edit", id = Model.GDSAdditionalEntryId }, FormMethod.Post, new { id = "form0" })){%>
        <%= Html.AntiForgeryToken() %>
    
        <table cellpadding="0" border="0" width="100%" cellspacing="0"> 
	        <tr> 
		        <th class="row_header" colspan="3">Edit GDS Additional Entry</th> 
	        </tr> 
           <tr>
           ...
           <tr>
                ...
                <td class="row_footer_blank_right"><input type="submit" value="Edit GDS Additional Entry" class="red" title="Edit GDS Additional Entry"/></td>
            </tr>
        </table>
            <%= Html.HiddenFor(model => model.VersionNumber) %>
            <%= Html.HiddenFor(model => model.GDSAdditionalEntryId) %>
            <%= Html.HiddenFor(model => model.SourceSystemCode)%>
            <%= Html.Hidden("SubProduct",Model.SubProductName)%>
