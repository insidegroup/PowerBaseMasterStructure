    ...
    <div id="banner"><div id="banner_text">Policy Supplier Deal Codes</div></div>
        ...
        <%using (Html.BeginForm()){%>
            ...
            <table cellpadding="0" cellspacing="0" width="100%"> 
		        <tr> 
			        <th class="row_header" colspan="3">Edit Policy Supplier Deal Code</th> 
		        </tr> 
                 <tr>
				...
				<tr class="tourcode-row">
                    ...
                    <td><%= Html.TextBoxFor(model => model.Endorsement)%></td>
                    <td>
						<%= Html.ValidationMessageFor(model => model.Endorsement)%>
						<span id="countEndorsement"></span> characters remaining</td>
                </tr> 
				<tr class="tourcode-row">
                    <td><label for="EndorsementOverride">Endorsement Override</label></td>
                    <td><%= Html.TextBoxFor(model => model.EndorsementOverride)%></td>
                    <td>
						<%= Html.ValidationMessageFor(model => model.EndorsementOverride)%>
						<span id="countEndorsementOverride"></span> characters remaining</td>
                </tr>
				<tr class="tourcode-row">
				...
				<% if (Model.PolicySupplierDealCodeOSIs.Count > 0) { 
					foreach(var policySupplierDealCodeOSI in Model.PolicySupplierDealCodeOSIs) { %>
						<tr class="PolicySupplierDealCodeOSILine" valign="top">
							<td><label id="PolicySupplierDealCodeOSILabel_<%=policySupplierDealCodeOSI.PolicySupplierDealCodeOSISequenceNumber %>">OSI <%=policySupplierDealCodeOSI.PolicySupplierDealCodeOSISequenceNumber %></label></td>
							<td><%= Html.TextArea("PolicySupplierDealCodeOSI_" + policySupplierDealCodeOSI.PolicySupplierDealCodeOSISequenceNumber.ToString(), policySupplierDealCodeOSI.PolicySupplierDealCodeOSIDescription, new { @class = "osi-input" }) %> 
							...
							<td><span id="PolicySupplierDealCodeOSI_Count_<%=policySupplierDealCodeOSI.PolicySupplierDealCodeOSISequenceNumber %>" class="character_count"></span> characters remaining</td>
						</tr>
					<% }
				} else { %>
				<tr class="PolicySupplierDealCodeOSILine" valign="top">
					<td><label id="PolicySupplierDealCodeOSILabel_1">OSI 1</label></td>
					<td>
                ...
                <tr>
                    ...
                    <td class="row_footer_blank_right"><input type="submit" value="Edit Policy Supplier Deal Code" class="red" title="Edit Policy Supplier Deal Code"/></td>
                </tr>
            </table>
