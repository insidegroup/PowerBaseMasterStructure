<%@ Page Title="" Language="C#" MasterPageFile="~/Views/Shared/SiteFullWidth.Master" Inherits="System.Web.Mvc.ViewPage<CWTDesktopDatabase.ViewModels.PolicyPriceTrackingOtherGroupItemDataTableItemVM>" %>
...
<div id="contentarea">
        ...
        <div id="content">
			...
			<% using (Html.BeginForm()) {%>
				...
				<table cellpadding="0" border="0" width="100%" cellspacing="0"> 
					...
					<% int counter = 1;
					foreach (var item in Model.PolicyPriceTrackingOtherGroupItemDataTableItems.OrderBy(x => x.PolicyOtherGroupHeaderColumnName.DisplayOrder)) { %>
						<tr>
							<td><label for="PolicyPriceTrackingOtherGroupItemLanguage_Label"><% =Html.Encode(item.PolicyOtherGroupHeaderColumnName.ColumnName)%></label></td>
							<td><%= Html.TextArea("PolicyOtherGroupHeaderColumnNameId_" + item.PolicyOtherGroupHeaderColumnNameId.ToString(), 
									...
									new { 
										...
										@class="PolicyPriceTrackingOtherGroupItemDataTableItem"
									})
								%>
							</td>
							<td>
								...
								<% if(counter == 1) { %>
									<span class="error" id="PolicyPriceTrackingOtherGroupItemDataTableItemErrorMessage"></span>
								<% } %>
							</td>
						</tr> 
					<% counter++; } %>
					...
					<tr>
						...
						<td class="row_footer_blank_right">
							...
							<%= Html.HiddenFor(model => model.PolicyPriceTrackingOtherGroupItemDataTableRow.PolicyPriceTrackingOtherGroupItemDataTableRowId)%>
							<%= Html.HiddenFor(model => model.PolicyPriceTrackingOtherGroupItemDataTableRow.VersionNumber)%>
	...
	<script src="<%=Url.Content("~/Scripts/ERD/PolicyPriceTrackingOtherGroupItemDataTableItem.js")%>" type="text/javascript"></script>
	<script type="text/javascript">
		$(document).ready(function () {
			<% foreach(var item in Model.PolicyPriceTrackingOtherGroupItemDataTableItems) { %>
...
<%=Html.RouteLink("Table Data", "Default", new { controller = "PolicyPriceTrackingOtherGroupItemDataTableItem", action = "List", id = Model.PolicyOtherGroupHeader.PolicyOtherGroupHeaderId, policyGroupId = Model.PolicyGroup.PolicyGroupId }, new { title = "Table Data" })%> &gt;
Edit
</asp:Content>