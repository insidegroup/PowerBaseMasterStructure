<%@ Page Title="" Language="C#" MasterPageFile="~/Views/Shared/ClientSummary.Master" Inherits="System.Web.Mvc.ViewPage<CWTDesktopDatabase.Helpers.CWTPaginatedList<CWTDesktopDatabase.Models.spDesktopDataAdmin_SelectClientSummaryClientTopUnitClientSubUnits_v1Result>>" %>
...
<div id="contentarea">
    <div id="banner"><div id="banner_text">Client Summary - Client SubUnits Matrix</div></div>
    <div id="content">
        <table cellpadding="0" cellspacing="0" border="0" style="width:2250px;">
            <tr> 
		        <th style="width:300px;">ClientSubUnit</th> 
            ...
            <tr>
                ...
                <% if (item.ClientSubUnitName != null){ %>
                    <%= Html.RouteLink(Html.Encode(item.ClientSubUnitName), "ListMain", new { controller = "ClientSubUnit", action = "ViewItem", id = item.ClientSubUnitGuid }, new { title = "View SubUnit Summary" })%>
                <%}%>
                </td>
                <td>
                <% if (item.ClientDetailId != null)
                   { %>
                    <%= Html.RouteLink(Html.Encode(item.ClientDetailName), "Main", new { controller = "ClientDetailClientSubUnit", action = "View", id = item.ClientDetailId }, new { title = "View ClientDetail" })%>
                <%}%>
                </td>
                <td>
