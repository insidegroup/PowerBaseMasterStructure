<%@ Page Title="" Language="C#" MasterPageFile="~/Views/Shared/Site.Master" Inherits="System.Web.Mvc.ViewPage<CWTDesktopDatabase.ViewModels.ClientSubUnitSubProductFormOfPaymentTypesVM>" %>
                    ...
                    <div class="paging_container">
                        <div class="paging_left">
                          <% if (Model.SubProductFormOfPaymentTypes.HasPreviousPage)
                             { %>
                        <%=  Html.RouteLink("<<Previous Page", "ListMain", new { controller = "ClientSubUnitSubProductFormOfPaymentTypeController", action = "List", id = Model.ClientDetail.ClientDetailId, page = (Model.SubProductFormOfPaymentTypes.PageIndex - 1) }, new { title = "Previous Page" })%>
                        <%}%>
                        </div>
                        <div class="paging_right">
                         <% if (Model.SubProductFormOfPaymentTypes.HasNextPage)
                            { %>
                        <%=  Html.RouteLink("Next Page>>", "ListMain", new { controller = "ClientSubUnitSubProductFormOfPaymentTypeController", action = "List", id = Model.ClientDetail.ClientDetailId, page = (Model.SubProductFormOfPaymentTypes.PageIndex + 1) }, new { title = "Next Page" })%>
                        <%}%> 
                        </div>
