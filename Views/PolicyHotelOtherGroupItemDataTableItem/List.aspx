<%@ Page Title="" Language="C#" MasterPageFile="~/Views/Shared/SiteFullWidth.Master" Inherits="System.Web.Mvc.ViewPage<CWTDesktopDatabase.ViewModels.PolicyHotelOtherGroupItemDataTableItemsVM>" %>
    ...
    <div id="banner"><div id="banner_text">Table Data Items</div></div>
        <div id="content">
			<table width="100%" cellpadding="0" cellspacing="0" border="0">
               <tr> 
			        <th class="row_header row_header_white" colspan="<%= Html.Encode(Model.PolicyHotelOtherGroupItemDataTableItems.Columns.Count + 1) %>">
						Table Name - <%= Html.Encode(Model.PolicyOtherGroupHeader.TableName) %>
			        </th> 
		        </tr>
				<tr>
					<% foreach (System.Data.DataColumn col in Model.PolicyHotelOtherGroupItemDataTableItems.Columns) { 
						if(col.ColumnName != "PolicyHotelOtherGroupItemDataTableRowId") { %>
				...
				<% foreach (System.Data.DataRow row in Model.PolicyHotelOtherGroupItemDataTableItems.Rows) { %>
					<tr>
						<% foreach (System.Data.DataColumn col in Model.PolicyHotelOtherGroupItemDataTableItems.Columns) { 
								if(col.ColumnName != "PolicyHotelOtherGroupItemDataTableRowId") { %>           
						...
						<td width="8%">
							<%if(Model.HasWriteAccess){%>
								<%=Html.RouteLink("Edit", "Default", new { action = "Edit", id = Html.Encode(row["PolicyHotelOtherGroupItemDataTableRowId"]), policyGroupId = Model.PolicyGroup.PolicyGroupId, PolicyOtherGroupHeaderId = Model.PolicyOtherGroupHeader.PolicyOtherGroupHeaderId }, new { title = "Edit" })%>
							<%} %>
						</td>
						<td width="10%">
							<%if(Model.HasWriteAccess){%>
								<%=Html.RouteLink("Delete", "Default", new { action = "Delete", id = Html.Encode(row["PolicyHotelOtherGroupItemDataTableRowId"]), policyGroupId = Model.PolicyGroup.PolicyGroupId, PolicyOtherGroupHeaderId = Model.PolicyOtherGroupHeader.PolicyOtherGroupHeaderId }, new { title = "Delete" })%>
							<%} %>
						</td>
					</tr>
				<% } %>
                <tr>
                    <td colspan="<%= Html.Encode(Model.PolicyHotelOtherGroupItemDataTableItems.Columns.Count + 1) %>" class="row_footer">
		        ...
		        <tr> 
					<td class="row_footer_blank_left" colspan="<%= Model.PolicyHotelOtherGroupItemDataTableItems.Columns.Count - 1 %>">
...
<script type="text/javascript">
	...
	$('#btnSearch').click(function () {
		...
		$("#frmSearch").attr("action", "/PolicyHotelOtherGroupItemDataTableItem.mvc/List");
		$("#frmSearch").submit();
	});
</script>
</asp:Content>
