<%@ Page Title="" Language="C#" MasterPageFile="~/Views/Shared/Site.Master" Inherits="System.Web.Mvc.ViewPage<CWTDesktopDatabase.Helpers.CWTPaginatedList<CWTDesktopDatabase.Models.spDesktopDataAdmin_SelectPolicyCarVendorGroupItemSequences_v1Result>>" %>
...
<div id="contentarea">
    <div id="banner"><div id="banner_text">Policy Car Vendor Group Items - Edit Sequencing</div></div>
        <div id="content">
        ...
        <% using (Html.BeginForm()) {%>
            ...
            <table width="100%" cellpadding="0" cellspacing="0" id="sortable">
                ...
                %>
                  <tr id="<%= Html.Encode(item.PolicyCarVendorGroupItemId)%>_<%= Html.Encode(item.VersionNumber) %>">
                    <td width="10%"><%= Html.Encode(item.SequenceNumber) %></td>
                    <td width="10%"><%= Html.Encode(item.PolicyLocationName) %></td>
                    <td width="10%"><%= Html.Encode(item.SupplierName) %></td>
                    <td width="10%"><%= Html.Encode(item.PolicyCarStatusDescription) %></td>
                    <td width="85%"><%= Html.Encode(item.EnabledDate.HasValue ? item.EnabledDate.Value.ToString("MMM dd, yyyy") : "")%></td>
		        ...
		        <tr> 
                 ...
                 <td class="row_footer_blank_right" colspan="3"><%= Html.ActionLink("Reset", "EditSequence", new { id = ViewData["PolicyGroupId"] }, new { @class = "red" })%>&nbsp;<input type="submit" value="Save" title="Save" class="red"/></td>
		        </tr> 
