...
/*
	...
	Thank you!
*/
g_jsDatePickImagePath = "../Images/WizardCalendar/";
...
JsDatePick.prototype.closeCalendar = function () {
    //modification for CWT
    var CWTId = $("#" + this.oConfiguration.target).attr('id');
    var CWTval = $("#" + this.oConfiguration.target).val();
    
    if (CWTId.indexOf("EnabledDate") > -1 && CWTval =="") {
        $("#" + this.oConfiguration.target).val("No Enabled Date");
    }
    if (CWTId.indexOf("ExpiryDate") > -1 && CWTval == "") {
        $("#" + this.oConfiguration.target).val("No Expiry Date");
    }
    if (CWTId.indexOf("TravelDateValidFrom") > -1 && CWTval == "") {
        $("#" + this.oConfiguration.target).val("No Travel Date Valid From");
    }
    if (CWTId.indexOf("TravelDateValidTo") > -1 && CWTval == "") {
        $("#" + this.oConfiguration.target).val("No Travel Date Valid To");
    }
    //end modification
	...
	if (this.oConfiguration.useMode == 2){
		if (this.oConfiguration.target != false){
			if (typeof($("#"+this.oConfiguration.target)) != null){
				...
				aSpan.style.zIndex = "2";
				this.initialZIndex++;
				
				JsDatePickBox.style.zIndex = this.initialZIndex.toString();
				JsDatePickBox.style.position = "absolute";
				JsDatePickBox.style.top = "0px";    //original:18px;
				JsDatePickBox.style.left = "130px"; //original:0px;
...
JsDatePick.prototype.determineFieldDate = function () {
    ...
    if (this.lastPickedDateObject) {
        ...
        //modification to allow some text values
        var CWTallowedtext = jQuery.trim(aField.val());
        if (CWTallowedtext == "No Enabled Date" || CWTallowedtext == "No Expiry Date" || CWTallowedtext == "No Travel Date Valid From" || CWTallowedtext == "No Travel Date Valid To") {
