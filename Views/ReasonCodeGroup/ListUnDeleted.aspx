...
<div id="contentarea">
    <div id="banner"><div id="banner_text">Reason Code Groups</div></div>
        <div id="content">
            <table cellpadding="0" cellspacing="0" border="0" style="width: 100%; table-layout: fixed;">
                <tr> 
			        <th width="25%" class="row_header_left"><%= Html.RouteLink("Reason Code Group Name", "ListMain", new { page =1, sortField = "ReasonCodeGroupName", sortOrder = ViewData["NewSortOrder"].ToString(), filter = Request.QueryString["filter"] })%></th> 
			        <th width="12%">&nbsp;</th>
                ...
                %>
                <tr>
                    <td colspan="2"><%= Html.Encode(CWTStringHelpers.TrimString(item.ReasonCodeGroupName, 40))%></td>
                    ...
                    <td>
                        <%if((bool)item.HasWriteAccess.Value == true){%>
                        <%=  Html.ActionLink("Delete", "Delete", new { id = item.ReasonCodeGroupId }, new { title = "Delete" })%>
                        <%} %>
                    </td>
                </tr>        
                <% 
		        ...
		        <tr>
			        ...
			        <td class="row_footer_blank_right" colspan="8"><%= Html.ActionLink("Orphaned Reason Code Groups", "ListOrphaned", null, new { @class = "red", title="Orphaned Reason Code Groups" })%>&nbsp;<%= Html.ActionLink("Deleted Reason Code Groups", "ListDeleted", null, new { @class = "red", title="Deleted Reason Code Groups"})%>&nbsp; <%if (ViewData["Access"] == "WriteAccess"){ %><%= Html.ActionLink("Create Reason Code Group", "Create", null, new { @class = "red" ,title="Create Reason Code Group" })%><%} %></td> 
		        </tr> 
            </table>
        </div>
    </div>
<script type="text/javascript">
    ...
    $('#btnSearch').click(function() {
      ...
      $("#frmSearch").attr("action", "/ReasonCodeGroup.mvc/ListUnDeleted");
      $("#frmSearch").submit();
