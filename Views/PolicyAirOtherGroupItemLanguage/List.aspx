<%@ Page Title="" Language="C#" MasterPageFile="~/Views/Shared/SiteFullWidth.Master" Inherits="System.Web.Mvc.ViewPage<CWTDesktopDatabase.ViewModels.PolicyAirOtherGroupItemLanguagesVM>" %>
    ...
    <div id="banner"><div id="banner_text">Translations</div></div>
        <div id="content">
            <table width="100%" cellpadding="0" cellspacing="0" border="0">
                ...
                <%
                foreach (var item in Model.PolicyAirVendorGroupItemLanguages) { 
                %>
                <tr>
					...
					<td width="10%">
						<%if(item.HasWriteAccess.Value){%>
							<%=  Html.RouteLink("Edit", "Default", new { action = "Edit", id = item.PolicyAirOtherGroupItemLanguageId, policyGroupId = item.PolicyGroupId, policyOtherGroupHeaderId = item.PolicyOtherGroupHeaderId }, new { title = "Edit" })%>
                        <%} %>
					</td>
					<td width="10%">
						<%if(item.HasWriteAccess.Value){%>
							<%=  Html.RouteLink("Delete", "Default", new { action = "Delete", id = item.PolicyAirOtherGroupItemLanguageId, policyGroupId = item.PolicyGroupId, policyOtherGroupHeaderId = item.PolicyOtherGroupHeaderId }, new { title = "Delete" })%>
                        <%} %>
					</td>
                </tr>        
                <% 
                ...
                <tr>
                    <td colspan="4" class="row_footer">
                        <div class="paging_container">
                            ...
                            <% if (Model.PolicyAirVendorGroupItemLanguages.HasPreviousPage){ %>
								<%=Html.RouteLink("<<Previous Page", "ListMain", new { action = "List", page = (Model.PolicyAirVendorGroupItemLanguages.PageIndex - 1), sortField = ViewData["CurrentSortField"], sortOrder = ViewData["CurrentSortOrder"].ToString(), filter = Request.QueryString["filter"] }, new { title = "Previous Page" })%>
                            <%}%>
                            </div>
                            <div class="paging_right">
                            <% if (Model.PolicyAirVendorGroupItemLanguages.HasNextPage){ %>
								<%=Html.RouteLink("Next Page>>", "ListMain", new { action = "List", page = (Model.PolicyAirVendorGroupItemLanguages.PageIndex + 1), sortField = ViewData["CurrentSortField"], sortOrder = ViewData["CurrentSortOrder"].ToString(), filter = Request.QueryString["filter"] }, new { title = "Next Page" })%>
                            <%}%> 
                            </div>
                            <div class="paging_centre"><%if (Model.PolicyAirVendorGroupItemLanguages.TotalPages > 0) { %>Page <%=Model.PolicyAirVendorGroupItemLanguages.PageIndex%> of <%=Model.PolicyAirVendorGroupItemLanguages.TotalPages%><%} %></div>
                        </div>
                    </td>
                </tr>
		        <tr> 
