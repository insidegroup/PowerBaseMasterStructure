...
$(document).ready(function() {
    ...
    $("#HierarchyType").change(function() {
        ...
        $("#HierarchyItem").val("");
        if ($("#TicketQueueGroupId").val() == "0") {
            $("#lblAuto").text("");
            $("#TicketQueueGroupName").val("");
        }
        if ($("#HierarchyType").val() == "") {
            $("#HierarchyItem").attr("disabled", true);
    ...
    $('#form0').submit(function () {
        ...
        if ($("#TicketQueueGroupId").val() == "0") {
            ...
            if (jQuery.trim($("#TicketQueueGroupName").val()) == "") {
                $("#TicketQueueGroupName_validationMessage").removeClass('field-validation-valid');
                $("#TicketQueueGroupName_validationMessage").addClass('field-validation-error');
                $("#TicketQueueGroupName_validationMessage").text("Ticket Q Group Name Required.");
                return false;
            } else {
                $("#TicketQueueGroupName_validationMessage").text("");
            }
        }
        ...
        jQuery.ajax({
            type: "POST",
            url: "/GroupNameBuilder.mvc/IsAvailableTicketQueueGroupName",
            data: { groupName: $("#TicketQueueGroupName").val(), id: $("#TicketQueueGroupId").val() },
            success: function(data) {
        ...
        if (!validGroupName) {
            ...
            $("#lblTicketQueueGroupNameMsg").removeClass('field-validation-valid');
            $("#lblTicketQueueGroupNameMsg").addClass('field-validation-error');
            if ($("#TicketQueueGroupId").val() == "0") {//Create
                $("#lblTicketQueueGroupNameMsg").text("This name has already been used, please reselect " + $("#lblHierarchyItem").text() + " to update the name.");
            } else {
                if ($("#TicketQueueGroupName").val() != "") {
                    $("#lblTicketQueueGroupNameMsg").text("This name has already been used, please choose a different name.");
                }
            }
            return false;
        } else {
            $("#lblTicketQueueGroupNameMsg").text("");
        }
        //GroupName End
        ...
        source: function(request, response) {
            ...
            if ($("#HierarchyType").val() != "ClientSubUnitTravelerType") {
                $.ajax({
                    ...
                    data: { searchText: request.term, hierarchyItem: $("#HierarchyType").val(), domainName: 'Queue', resultCount: 5000 },
                    success: function (data) {
                ...
                $.ajax({
                    url: "/TicketQueueGroup.mvc/AutoCompleteClientSubUnitTravelerTypes", type: "POST", dataType: "json",
            ...
            if ($("#TicketQueueGroupId").val() == "0") {//Create
                ...
                if ($("#HierarchyType").val() == "ClientAccount") {
                    $.ajax({
                        ...
                        data: { clientAccountNumber: $("#HierarchyCode").val(), sourceSystemCode: $("#SourceSystemCode").val(), group: "Ticket Queue" },
                        success: function(data) {
                            var maxNameSize = 50 - ($("#HierarchyType").val().length + 17);
                            var autoName = replaceSpecialChars(ui.item.value)
                            autoName = autoName.substring(0, maxNameSize) + "_" + data + "_" + $("#HierarchyType").val() + "_TicketQueue";
                            $("#lblAuto").text(autoName);
                            $("#TicketQueueGroupName").val(autoName);
                            $("#lblTicketQueueGroupNameMsg").text("");
                        }
                    })
                } else {
                    $.ajax({
                        ...
                        data: { hierarchyType: $("#HierarchyType").val(), hierarchyItem: $("#HierarchyCode").val(), group: "Ticket Queue" },
                        success: function(data) {
                            var maxNameSize = 50 - ($("#HierarchyType").val().length + 17);
                            var autoName = replaceSpecialChars(ui.item.value)
                            autoName = autoName.substring(0, maxNameSize) + "_" + data + "_" + $("#HierarchyType").val() + "_TicketQueue";
