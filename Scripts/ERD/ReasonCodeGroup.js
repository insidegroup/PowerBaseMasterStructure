...
$(document).ready(function () {
    ...
    $("#HierarchyType").change(function () {
        ...
        $("#HierarchyItem").val("");
        if ($("#ReasonCodeGroupId").val() == "0") {
            $("#lblAuto").text("");
            $("#ReasonCodeGroupName").val("");
        }
        if ($("#HierarchyType").val() == "") {
            $("#HierarchyItem").attr("disabled", true);
    ...
    $('#form0').submit(function () {
        ...
        if ($("#ReasonCodeGroupId").val() == "0") {
            ...
            if (jQuery.trim($("#ReasonCodeGroupName").val()) == "") {
                $("#ReasonCodeGroupName_validationMessage").removeClass('field-validation-valid');
                $("#ReasonCodeGroupName_validationMessage").addClass('field-validation-error');
                $("#ReasonCodeGroupName_validationMessage").text("Reason Code Group Name Required.");
                return false;
            } else {
                $("#ReasonCodeGroupName_validationMessage").text("");
            }
        }
        ...
        jQuery.ajax({
            type: "POST",
            url: "/GroupNameBuilder.mvc/IsAvailableReasonCodeGroupName",
            data: { groupName: $("#ReasonCodeGroupName").val(), id: $("#ReasonCodeGroupId").val() },
            success: function (data) {
                validGroupName = data;
            },
            dataType: "json",
        ...
        if (!validGroupName) {
            ...
            $("#lblReasonCodeGroupNameMsg").removeClass('field-validation-valid');
            $("#lblReasonCodeGroupNameMsg").addClass('field-validation-error');
            if ($("#ReasonCodeGroupId").val() == "0") {//Create
                $("#lblReasonCodeGroupNameMsg").text("This name has already been used, please reselect " + $("#lblHierarchyItem").text() + " to update the name.");
            } else {
                if ($("#ReasonCodeGroupName").val() != "") {
                    $("#lblReasonCodeGroupNameMsg").text("This name has already been used, please choose a different name.");
                }
            }
            return false;
        } else {
            $("#lblReasonCodeGroupNameMsg").text("");
        }
        //GroupName End
    ...
    $("#HierarchyItem").autocomplete({
        source: function (request, response) {
            ...
            if ($("#HierarchyType").val() == "ClientSubUnitTravelerType") {
            	$.ajax({
            		url: "/ReasonCodeGroup.mvc/AutoCompleteClientSubUnitTravelerTypes", type: "POST", dataType: "json",
            	...
            	$.ajax({
            		...
            		data: { searchText: request.term, hierarchyItem: "TravelerType", domainName: 'Reason Code', resultCount: 5000 },
            		success: function (data) {
            			response($.map(data, function (item) {
            				return {
            	...
            	$.ajax({
            		url: "/AutoComplete.mvc/Hierarchies", type: "POST", dataType: "json",
            		data: { searchText: request.term, hierarchyItem: $("#HierarchyType").val(), domainName: 'Reason Code', resultCount: 5000 },
            		success: function (data) {
            			response($.map(data, function (item) {
            				if (
            ...
            if ($("#ReasonCodeGroupId").val() == "0") {//Create
                ...
                if ($("#HierarchyType").val() == "ClientAccount") {
                    $.ajax({
                        ...
                        data: { clientAccountNumber: $("#HierarchyCode").val(), sourceSystemCode: $("#SourceSystemCode").val(), group: "Reason Code" },
                        success: function (data) {
                            var maxNameSize = 50 - (htft.length + 16);
                            var autoName = replaceSpecialChars(ui.item.value)
                            autoName = autoName.substring(0, maxNameSize) + "_" + data + "_" + htft + "_ReasonCode";
                            $("#lblAuto").text(autoName);
                            $("#ReasonCodeGroupName").val(autoName);
                            $("#lblReasonCodeGroupNameMsg").text("");
                        }
                    })
                } else {
                    $.ajax({
                        ...
                        data: { hierarchyType: $("#HierarchyType").val(), hierarchyItem: $("#HierarchyCode").val(), group: "Reason Code" },
                        success: function (data) {
