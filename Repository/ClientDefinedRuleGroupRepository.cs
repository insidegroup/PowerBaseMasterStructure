...
{
	public class ClientDefinedRuleGroupRepository
    {
        ...
        public CWTPaginatedList<spDesktopDataAdmin_SelectClientDefinedRuleGroups_v1Result> PageClientDefinedRuleGroups(
			bool deleted, 
			...
			int sortOrder)
        {
			...
			var result = db.spDesktopDataAdmin_SelectClientDefinedRuleGroups_v1(deleted, clientTopUnitName, clientSubUnitName, clientAccountNumber, sourceSystemCode, travelerTypeName, clientDefinedRuleGroupName, filter, sortField, sortOrder, page, adminUserGuid).ToList();
            ...
            var paginatedView = new CWTPaginatedList<spDesktopDataAdmin_SelectClientDefinedRuleGroups_v1Result>(result, page, totalRecords);

            //return to user
            return paginatedView;
        }

		//List of ClientDefinedRuleGroups Sequences
		public CWTPaginatedList<spDesktopDataAdmin_SelectClientDefinedRuleGroupSequences_v1Result> PageClientDefinedRuleGroupSequences(
			string clientTopUnitName,
			string clientSubUnitName,
			string clientAccountNumber,
			string sourceSystemCode,
			...
			int page)
		{
			string adminUserGuid = HttpContext.Current.User.Identity.Name.Split(new[] { '|' })[0];
			int pageSize = 50;
			var result = db.spDesktopDataAdmin_SelectClientDefinedRuleGroupSequences_v1(
				clientTopUnitName,
				clientSubUnitName,
				clientAccountNumber,
				sourceSystemCode,
			...
			var paginatedView = new CWTPaginatedList<spDesktopDataAdmin_SelectClientDefinedRuleGroupSequences_v1Result>(result, page, Convert.ToInt32(totalRecords));
			return paginatedView;
		...
		//Get one ClientDefinedRuleGroup
		...
		public Dictionary<string, string> GetSearchListFilters()
		{
			Dictionary<string, string> searchFields = new Dictionary<string, string>() {
				 { "ClientTopUnit", "Client TopUnit Name" },
				 { "ClientSubUnit",  "Client SubUnit Name" },
				 { "ClientAccount",  "Client Account" },
				 { "TravelerType",  "Client Traveler Type" },
				 { "ClientSubUnitTravelerType",  "Client SubUnit Traveler Type" }
			};
		...
		public void Edit(ClientDefinedRuleGroupVM clientDefinedRuleGroupVM)
		{
			...
			db.spDesktopDataAdmin_UpdateClientDefinedRuleGroup_v1(
				...
				System.Xml.Linq.XElement.Parse(clientDefinedRuleResultItemDoc.OuterXml),
				System.Xml.Linq.XElement.Parse(clientDefinedRuleWorkflowTriggerDoc.OuterXml),
				clientDefinedRuleGroupVM.ClientDefinedRuleGroup.IsMultipleHierarchy,
				adminUserGuid
			);
		}
		...
		public List<spDesktopDataAdmin_SelectClientDefinedRuleGroupLinkedHierarchies_v1Result> ClientDefinedRuleGroupLinkedHierarchies(int clientDefinedRuleGroupId, string filterHierarchyType)
		{
			return db.spDesktopDataAdmin_SelectClientDefinedRuleGroupLinkedHierarchies_v1(clientDefinedRuleGroupId, filterHierarchyType).ToList();
		}
		...
		//Get Hierarchies linked to of ClientDefinedRuleGroups
		public List<spDesktopDataAdmin_SelectClientDefinedRuleGroupAvailableHierarchies_v1Result> ClientDefinedRuleGroupAvailableHierarchies(int clientDefinedRuleGroupId, string hierarchyType, string filter, string clientSubUnit = "", string travelerType = "")
		{
			//get a page of records
			string adminUserGuid = HttpContext.Current.User.Identity.Name.Split(new[] { '|' })[0];
			return db.spDesktopDataAdmin_SelectClientDefinedRuleGroupAvailableHierarchies_v1(clientDefinedRuleGroupId, hierarchyType, filter, clientSubUnit, travelerType, adminUserGuid).ToList();
		}

		//Change the deleted status on an item
		public void UpdateLinkedHierarchy(ClientDefinedRuleGroupHierarchyVM groupHierarchyVM)
		{
			string adminUserGuid = HttpContext.Current.User.Identity.Name.Split(new[] { '|' })[0];

			db.spDesktopDataAdmin_UpdateClientDefinedRuleGroupLinkedHierarchy_v1(
		...
		public SelectList GetHierarchyPropertyOptions(string groupName, bool isBusiness, string selectedValue = "") {
