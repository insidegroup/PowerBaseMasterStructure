...
<div id="contentarea">
    <div id="banner"><div id="banner_text">Policy Air Time Window Group Items</div></div>
        <div id="content">
             <table width="100%" cellpadding="0" cellspacing="0" border="0">
                <tr> 
                    <th width="20%" class="row_header_left"><%=  Html.RouteLink("Time Window Minutes", "List", new { controller = "PolicyAirTimeWindowGroupItem", action = "List", id = Model.PolicyGroup.PolicyGroupId, page = 1, sortField = "PolicyAirParameterValue", sortOrder = ViewData["NewSortOrder"].ToString() }, new { title = "Sort By Time Window Minutes" })%></th>
                    <th width="22%"><%=  Html.RouteLink("Routing", "List", new { controller = "PolicyAirTimeWindowGroupItem", action = "List", id = Model.PolicyGroup.PolicyGroupId, page = 1, sortField = "Name", sortOrder = ViewData["NewSortOrder"].ToString() }, new { title = "Sort By Routing Name" })%></th>
                    <th width="27%" colspan="2">Travel date valid from/to</th>
                    <th width="17%"><%=  Html.RouteLink("Advice", "List", new { controller = "PolicyAirTimeWindowGroupItem", action = "List", id = Model.PolicyGroup.PolicyGroupId, page = 1, sortField = "Advice", sortOrder = ViewData["NewSortOrder"].ToString() }, new { title = "Sort By Advice" })%></th>
			        <th width="5%">&nbsp;</th> 
			        <th width="4%">&nbsp;</th> 
			        <th width="4%">&nbsp;</th> 
                ...
                <tr>
                    ...
                    <td><%= Html.RouteLink("Advice", "List", new { controller = "PolicyAirTimeWindowGroupItemLanguage", action = "List", id = item.PolicyAirParameterGroupItemId, Model.PolicyGroup.PolicyGroupId }, new { title = "Advice" })%></td>
                    ...
                    <div class="paging_container">
                        <div class="paging_left">
							<% if (Model.PolicyAirParameterGroupItems.HasPreviousPage){ %>
								<%=  Html.RouteLink("<<Previous Page", "ListMain", new { controller = "PolicyAirTimeWindowGroupItem", action = "List", id = Model.PolicyGroup.PolicyGroupId, page = (Model.PolicyAirParameterGroupItems.PageIndex - 1), sortField = ViewData["CurrentSortField"], sortOrder = ViewData["CurrentSortOrder"].ToString() }, new { title = "Previous Page" })%>
							<%}%>
                        </div>
                        <div class="paging_right">
							<% if (Model.PolicyAirParameterGroupItems.HasNextPage){ %>
								<%=  Html.RouteLink("Next Page>>", "ListMain", new { controller = "PolicyAirTimeWindowGroupItem", action = "List", id = Model.PolicyGroup.PolicyGroupId, page = (Model.PolicyAirParameterGroupItems.PageIndex + 1), sortField = ViewData["CurrentSortField"], sortOrder = ViewData["CurrentSortOrder"].ToString() }, new { title = "Next Page" })%>
							<%}%> 
                        </div>
		    ...
		    <tr> 
				...
				<td class="row_footer_blank_right" colspan="8">
					<%if (Model.HasWriteAccess) { %>
						<%= Html.RouteLink("Create Policy Air Time Window Group Item", "Default", new { action = "Create", id = Model.PolicyGroup.PolicyGroupId }, new { @class = "red", title = "Create Policy Air Time Window Group Item" })%>
					<% } %> 
			    </td> 
		    </tr> 
        </table>    
...
Policy Air Time Window Group Items
</asp:Content>