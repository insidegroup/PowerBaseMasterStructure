...
<div id="contentarea">
    <div id="banner"><div id="banner_text">Approval Groups</div></div>
        <div id="content">
            <table cellpadding="0" cellspacing="0" border="0" style="width: 100%; table-layout: fixed;">
                <tr> 
			        <th width="30%" class="row_header_left"><%= Html.RouteLink("Group Name", "ListMain", new { page =1, sortField = "ApprovalGroupName", sortOrder = ViewData["NewSortOrder"].ToString(), filter = Request.QueryString["filter"] })%></th> 
			        ...
			        <th width="20%">Approval Type(s)</th> 
			        <th width="5%">&nbsp;</th> 
			        <th width="5%">&nbsp;</th> 
			        <th width="4%">&nbsp;</th> 
                ...
                %>
                <tr>
                    <td class="wrap-text"><%= Html.Encode(CWTStringHelpers.TrimString(item.ApprovalGroupName, 40))%></td>
                    <td><%= Html.Encode(item.HierarchyType) %></td>
                    <td><%if(item.EnabledDate != null) { %>
							<%= Html.Encode(string.Format("{0:MMM dd, yyyy}", item.EnabledDate)) %>
						<% } %>
                    </td>
                    <td><%= Html.Encode(item.ApprovalGroupApprovalTypeItems) %></td>
                    ...
                    <td>
                        <%if((bool)item.HasWriteAccess.Value == true){%>
                        <%=  Html.ActionLink("Delete", "Delete", new { id = item.ApprovalGroupId }, new { title = "Delete" })%>
                        <%} %>
                    </td>
                </tr>        
                <% 
		        ...
		        <tr>
                    <td colspan="4" class="row_footer_blank_left">
						<a href="javascript:window.print();" class="red btn-small" title="Print">Print</a> 
						<%= Html.ActionLink("Orphaned Approval Groups", "ListOrphaned", null, new { @class = "red btn-small", title="Orphaned Approval Groups" })%>&nbsp;
						<%= Html.ActionLink("Deleted Approval Groups", "ListDeleted", null, new { @class = "red btn-small", title="Deleted Approval Groups"})%>&nbsp;
                    </td>
                     <td class="row_footer_blank_right" colspan="4">
						<%if (ViewData["Access"] == "WriteAccess"){ %>
							<%= Html.ActionLink("Create Approval Group", "Create", null, new { @class = "red btn-small" ,title="Create Approval Group" })%>
						<%} %>
                     </td>
		        </tr> 
            </table>
...
<script type="text/javascript">
    ...
    $('#btnSearch').click(function() {
      ...
      $("#frmSearch").attr("action", "/ApprovalGroup.mvc/ListUnDeleted");
      $("#frmSearch").submit();
