...
<div id="contentarea">
    <div id="banner"><div id="banner_text">Optional Field Groups (Orphaned) </div></div>
    <div id="content">
        <table width="100%" cellpadding="0" cellspacing="0">
            ...
            <%
            foreach (var item in Model.OptionalFieldGroupsOrphaned) { 
            %>
            <tr>
                ...
                <td><%= Html.RouteLink("Items", "List", new { id = item.OptionalFieldGroupId, controller="OptionalFieldItem" }, new { title="Client Fee Items"})%></td>
                ...
                <td>
                    <%if(item.HasWriteAccess.Value){%>
                        <%if(item.DeletedFlag){%>
                            <%=Html.RouteLink("UnDelete", "Default", new { action = "UnDelete", id = item.OptionalFieldGroupId }, new { title = "UnDelete" })%>
                        <%}else{%>
            ...
            <tr>
                <td colspan="7" class="row_footer">
                    <div class="paging_container">
                        <div class="paging_left"><% if (Model.OptionalFieldGroupsOrphaned.HasPreviousPage){ %><%=  Html.RouteLink("<<Previous Page", "ListMain", new { action = "ListOrphaned", page = (Model.OptionalFieldGroupsOrphaned.PageIndex - 1), sortField = ViewData["CurrentSortField"], sortOrder = ViewData["CurrentSortOrder"].ToString(), filter = Request.QueryString["filter"] }, new { title = "Previous Page" })%><%}%></div>
                        <div class="paging_right"><% if (Model.OptionalFieldGroupsOrphaned.HasNextPage){ %><%=  Html.RouteLink("Next Page>>", "ListMain", new { action = "ListOrphaned", page = (Model.OptionalFieldGroupsOrphaned.PageIndex + 1), sortField = ViewData["CurrentSortField"], sortOrder = ViewData["CurrentSortOrder"].ToString(), filter = Request.QueryString["filter"] }, new { title = "Next Page" })%><%}%></div>
                        <div class="paging_centre"><%if (Model.OptionalFieldGroupsOrphaned.TotalPages > 0){ %>Page <%=Model.OptionalFieldGroupsOrphaned.PageIndex%> of <%=Model.OptionalFieldGroupsOrphaned.TotalPages%><%} %></div>
                    </div>
                </td>
            </tr>
		    <tr>   
...
<script type="text/javascript">
    ...
    $('#btnSearch').click(function () {
        ...
        $("#frmSearch").attr("action", "/OptionalFieldGroup.mvc/ListOrphaned");
        $("#frmSearch").submit();
