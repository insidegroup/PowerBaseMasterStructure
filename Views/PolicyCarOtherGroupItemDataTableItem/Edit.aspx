<%@ Page Title="" Language="C#" MasterPageFile="~/Views/Shared/SiteFullWidth.Master" Inherits="System.Web.Mvc.ViewPage<CWTDesktopDatabase.ViewModels.PolicyCarOtherGroupItemDataTableItemVM>" %>
...
<div id="contentarea">
        ...
        <div id="content">
			...
			<% using (Html.BeginForm()) {%>
				...
				<table cellpadding="0" border="0" width="100%" cellspacing="0"> 
					...
					<% int counter = 1;
					foreach(var item in Model.PolicyCarOtherGroupItemDataTableItems.OrderBy(x => x.PolicyOtherGroupHeaderColumnName.DisplayOrder)) { %>
						<tr>
							<td><label for="PolicyCarOtherGroupItemLanguage_Label"><% =Html.Encode(item.PolicyOtherGroupHeaderColumnName.ColumnName)%></label></td>
							<td><%= Html.TextArea("PolicyOtherGroupHeaderColumnNameId_" + item.PolicyOtherGroupHeaderColumnNameId.ToString(), 
									...
									new { 
										...
										@class="PolicyCarOtherGroupItemDataTableItem"
									}
								)%>
							</td>
							<td>
								...
								<% if(counter == 1) { %>
									<span class="error" id="PolicyCarOtherGroupItemDataTableItemErrorMessage"></span>
								<% } %>
							</td>
						</tr> 
					<% counter++; } %>
					...
					<tr>
						...
						<td class="row_footer_blank_right">
							...
							<%= Html.HiddenFor(model => model.PolicyCarOtherGroupItemDataTableRow.PolicyCarOtherGroupItemDataTableRowId)%>
							<%= Html.HiddenFor(model => model.PolicyCarOtherGroupItemDataTableRow.VersionNumber)%>
	...
	<script src="<%=Url.Content("~/Scripts/ERD/PolicyCarOtherGroupItemDataTableItem.js")%>" type="text/javascript"></script>
...
<%=Html.RouteLink("Table Data", "Default", new { controller = "PolicyCarOtherGroupItemDataTableItem", action = "List", id = Model.PolicyOtherGroupHeader.PolicyOtherGroupHeaderId, policyGroupId = Model.PolicyGroup.PolicyGroupId }, new { title = "Table Data" })%> &gt;
Edit
</asp:Content>