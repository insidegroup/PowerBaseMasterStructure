<%@ Page Title="" Language="C#" MasterPageFile="~/Views/Shared/SiteFullWidth.Master" Inherits="System.Web.Mvc.ViewPage<CWTDesktopDatabase.ViewModels.PolicyAllOtherGroupItemDataTableItemVM>" %>
...
<div id="contentarea">
        ...
        <div id="content">
			...
			<% using (Html.BeginForm()) {%>
				...
				<table cellpadding="0" border="0" width="100%" cellspacing="0"> 
					...
					<% int counter = 1;
					foreach(var item in Model.PolicyAllOtherGroupItemDataTableItems.OrderBy(x => x.PolicyOtherGroupHeaderColumnName.DisplayOrder)) { %>
						<tr>
							<td><label for="PolicyAllOtherGroupItemLanguage_Label"><% =Html.Encode(item.PolicyOtherGroupHeaderColumnName.ColumnName)%></label></td>
							<td><%= Html.TextArea("PolicyOtherGroupHeaderColumnNameId_" + item.PolicyOtherGroupHeaderColumnNameId.ToString(),
									...
									new { 
										...
										@class="PolicyAllOtherGroupItemDataTableItem"
									})
								%>
							</td>
							<td>
								...
								<% if(counter == 1) { %>
									<span class="error" id="PolicyAllOtherGroupItemDataTableItemErrorMessage"></span>
								<% } %>
							</td>
						</tr> 
					<% counter++; } %>
					...
					<tr>
						...
						<td class="row_footer_blank_right">
							...
							<%= Html.HiddenFor(model => model.PolicyAllOtherGroupItemDataTableRow.PolicyAllOtherGroupItemDataTableRowId)%>
							<%= Html.HiddenFor(model => model.PolicyAllOtherGroupItemDataTableRow.VersionNumber)%>
	...
	<script src="<%=Url.Content("~/Scripts/ERD/PolicyAllOtherGroupItemDataTableItem.js")%>" type="text/javascript"></script>
...
<%=Html.RouteLink("Table Data", "Default", new { controller = "PolicyAllOtherGroupItemDataTableItem", action = "List", id = Model.PolicyOtherGroupHeader.PolicyOtherGroupHeaderId, policyGroupId = Model.PolicyGroup.PolicyGroupId }, new { title = "Table Data" })%> &gt;
Edit
</asp:Content>