        ...
        <div id="content">
			...
			<table width="100%" cellpadding="0" cellspacing="0">
                ...
                <tr>
                    ...
                    <td>
                        <%if (item.HasWriteAccess == true){ %>
							<%=  Html.ActionLink("Delete", "Delete", new { id = item.PseudoCityOrOfficeMaintenanceId})%>
                        <%} %>
                    </td>
                </tr>        
                <% 
						...
						<div class="paging_container">
							<div class="paging_left">
								<% if (Model.PseudoCityOrOfficeMaintenances.HasPreviousPage){ %>
									<%=  Html.RouteLink("<<Previous Page", "ListMain", new { controller = "PseudoCityOrOfficeMaintenance", action = "ListUnDeleted", page = (Model.PseudoCityOrOfficeMaintenances.PageIndex - 1), sortField = ViewData["CurrentSortField"].ToString(), sortOrder = ViewData["CurrentSortOrder"].ToString() }, new { title = "Previous Page" })%>
								<%}%>
							</div>
							<div class="paging_right">
								<% if (Model.PseudoCityOrOfficeMaintenances.HasNextPage){ %>
									<%=  Html.RouteLink("Next Page>>", "ListMain", new { controller = "PseudoCityOrOfficeMaintenance", action = "ListUnDeleted", page = (Model.PseudoCityOrOfficeMaintenances.PageIndex + 1), sortField = ViewData["CurrentSortField"].ToString(), sortOrder = ViewData["CurrentSortOrder"].ToString() }, new { title = "Next Page" })%>
								<%}%> 
							</div>
		        ...
		        <tr> 
                    <td class="row_footer_blank_left" colspan="2">
                    	...
                    	<%= Html.ActionLink("Export", "Export", new { }, new { @class = "red", title = "Export" })%> 
					</td>
			        <td class="row_footer_blank_right" colspan="10">
						<%= Html.ActionLink("Deleted PCC/OIDs", "ListDeleted", new { }, new { @class = "red", title = "Deleted PCC/OIDs" })%>
						<%	if (ViewData["Access"] == "WriteAccess"){ %>
							<%= Html.ActionLink("Create PCC/OIDs", "Create", new { }, new { @class = "red", title = "Create PCC/OIDs" })%>
						<%}%>
			        </td> 
		        </tr> 
            </table>
...
<script type="text/javascript">
    $(document).ready(function () {
        ...
        $('#btnSearch').click(function () {
        	...
        	$("#frmSearch").attr("action", "/PseudoCityOrOfficeMaintenance.mvc/ListUnDeleted");
        	$("#frmSearch").submit();
        });
    })
 </script>
