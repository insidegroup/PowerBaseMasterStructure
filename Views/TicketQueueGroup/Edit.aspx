<%@ Page Title="" Language="C#" MasterPageFile="~/Views/Shared/Site.Master" Inherits="System.Web.Mvc.ViewPage<CWTDesktopDatabase.Models.TicketQueueGroup>" %>

<asp:Content ID="Content1" ContentPlaceHolderID="TitleContent" runat="server">DesktopDataAdmin - Queues</asp:Content>

<asp:Content ID="Content2" ContentPlaceHolderID="MainContent" runat="server">
<div id="contentarea">
    <div id="banner"><div id="banner_text">Ticket Queue Group</div></div>
        <div id="content">
        <% Html.EnableClientValidation(); %>
        <% Html.EnableUnobtrusiveJavaScript(); %>
        <%using (Html.BeginRouteForm("Default", new { controller = "TicketQueueGroup", action = "Edit", id = Model.TicketQueueGroupId }, FormMethod.Post, new { id = "form0", autocomplete="off" })) {%>
        <%= Html.AntiForgeryToken() %>
            <table cellpadding="0" cellspacing="0" width="100%"> 
		        <tr> 
			        <th class="row_header" colspan="3">Edit Ticket Queue Group</th> 
		        </tr>
		         <tr>
                    <td><label for="TicketQueueGroupName">Ticket Queue Group Name</label></td>
                    <td><%= Html.TextBoxFor(model => model.TicketQueueGroupName, new { maxlength = "50", autocomplete="off" })%><span class="error"> *</span></td>
                    <td><%= Html.ValidationMessageFor(model => model.TicketQueueGroupName)%><label id="lblTicketQueueGroupNameMsg"/></td>
                </tr> 
                 <tr>
                    <td><label for="EnabledFlag">Enabled?</label></td>
                    <td> <%= Html.EditorFor(model => model.EnabledFlag, new { autocomplete="off" })%></td>
               ...
               <tr>
                    ...
                    <td class="row_footer_blank_right"><input type="submit" value="Edit Ticket Queue Group" class="red" title="Edit Ticket Queue Group"/></td>
                </tr>
            </table>
...
<script src="<%=Url.Content("~/Scripts/ERD/TicketQueueGroup.js")%>" type="text/javascript"></script>
</asp:Content>
